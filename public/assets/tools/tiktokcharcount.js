(function () {
  ...
  function analyze() {
    ...
    var readTime = words < 40 ? T('lessthanminute','less than a minute') : Math.ceil(words / 200) + T('minread',' min read');
    ...
    out.innerHTML =
      ...
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:0.9rem">' +
        '<div>🏷 '+T('hashtagslabel','Hashtags:')+' <strong>' + hashtags + '</strong></div>' +
        '<div>📝 '+T('wordslabel','Words:')+' <strong>' + words + '</strong></div>' +
        '<div>⏱ '+T('readtimelabel','Read time:')+' <strong>' + readTime + '</strong></div>' +
        '<div>✅ '+T('remaininglabel','Remaining:')+' <strong>' + Math.max(0, 2200 - chars) + '</strong></div>' +
      '</div>' +
