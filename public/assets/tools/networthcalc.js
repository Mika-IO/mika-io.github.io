(function(){
  ...
  function calc(){
    ...
    res.innerHTML=[['Total assets','$'+assets.toLocaleString((window.__LANG||"en"),{minimumFractionDigits:2,maximumFractionDigits:2})],['Total liabilities','$'+liabs.toLocaleString((window.__LANG||"en"),{minimumFractionDigits:2,maximumFractionDigits:2})],['Net worth','$'+nw.toLocaleString((window.__LANG||"en"),{minimumFractionDigits:2,maximumFractionDigits:2})]].map(([k,v])=>`<div style="background:var(--surface);border:1px solid var(--line);border-radius:8px;padding:0.5rem 0.75rem"><div style="font-size:0.75rem;opacity:0.6">${k}</div><strong>${v}</strong></div>`).join('');
  }
